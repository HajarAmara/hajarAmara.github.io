...
<%@page import=" java.io.UnsupportedEncodingException,com.hajar.zaki.dao.UserDAO, com.hajar.zaki.dao.UserDAOImpl,
 				  com.hajar.zaki.model.User , utils.*, java.sql.*, java.util.*,com.hajar.zaki.dao.ProductDAO,
				  com.hajar.zaki.model.Product,com.hajar.zaki.dao.ProductCustomerDAO,com.hajar.zaki.model.ProductCustomer, 
 				  com.hajar.zaki.db.DBConnect, com.hajar.zaki.model.ProductPicture, com.hajar.zaki.dao.PicturesDao, java.io.*, java.util.Base64,
 					com.hajar.zaki.model.Order, com.hajar.zaki.dao.SupplierDAO,com.hajar.zaki.dao.OrderDAO" %>
...
<div class="menu">
	...
	<div class="menu_nav">
		<ul>
					...
					<li class="active"><a href="sweet.jsp?userName=<%=userName%>&email=<%=email%>"  >קינוח</a></li>
		...
		<div class="home">
			<div class="home_container d-flex flex-column align-items-center justify-content-end">
				<div class="home_content text-center">
					<div class="home_title">קינוח</div>
		...
		<div class="products">
			<div class="container">
				...
				<% if(email != null && sup){ %>
					...
					ArrayList<Product> sweets_sup= prodDao.getAllSWEETS_SUP(email);
					int valuesLength=sweets_sup.size()-1;
				    InputStream sImage;
					int index_Prod=0;
				    for(index_Prod=0; index_Prod<=valuesLength;index_Prod++){
					  PicturesDao picDao= new PicturesDao();
					  ProductPicture prodPic = picDao.getPicturePerProductId(sweets_sup.get(index_Prod).getProdID());
					  String imageBase64="";
						...
						<div class="product">
							...
							<div class="product_content">
								<div class="product_info d-flex flex-row align-items-start justify-content-start">
									<div>
										<div>
											<div class="product_name"><a href="product.jsp?value=<%=sweets_sup.get(index_Prod).getProdID()%>">
											 <% 
										        out.println(sweets_sup.get(index_Prod).getName() ); %> </a></div>
											<div class="product_category">המחיר / السعر  </div>
										</div>
									</div>
									<div class="ml-auto text-right">
										<!-- div class="rating_r rating_r_4 home_item_rating"><i></i><i></i><i></i><i></i><i></i></div-->
										<div class="product_price text-right">₪<%out.println(sweets_sup.get(index_Prod).getPiece_price() ); %> </div>
									</div>
								</div>
				...
				<div class="row products_row ">
					...
					<% 
					ProductDAO prodDao=new ProductDAO();
					ArrayList<Product> sweets= prodDao.getAllSweets();
					int valuesLength=sweets.size()-1;
				    InputStream sImage;
					int index_Prod=0;
				    for(index_Prod=0; index_Prod<=valuesLength;index_Prod++){
					  PicturesDao picDao= new PicturesDao();
					  ProductPicture prodPic = picDao.getPicturePerProductId(sweets.get(index_Prod).getProdID());
					  String imageBase64="";
					  if(prodPic != null && prodPic.getImg()!= null){
						  System.out.print("test into index img " + imageBase64);
						  imageBase64 = new String(Base64.getEncoder().encode((prodPic.getImg())));
						...
						<div class="product">
							...
							<div class="product_content">
								<div class="product_info d-flex flex-row align-items-start justify-content-start">
									<div>
										<div>
											<div class="product_name"><a href="product.jsp?value=<%=sweets.get(index_Prod).getProdID()%>">
											 <% 
										        out.println(sweets.get(index_Prod).getName() ); %> </a></div>
											<div class="product_category">המחיר / السعر  </div>
										</div>
									</div>
									<div class="ml-auto text-right">
										<!-- div class="rating_r rating_r_4 home_item_rating"><i></i><i></i><i></i><i></i><i></i></div-->
										<div class="product_price text-right">₪<%out.println(sweets.get(index_Prod).getPiece_price() ); %> </div>
									</div>
								</div>
								<%  if( sweets.get(index_Prod) != null){
								      ProductCustomerDAO pcdao = new ProductCustomerDAO(); 
									  ProductCustomer pc = new ProductCustomer(); 
									  pc = pcdao.getProductLike(sweets.get(index_Prod).getProdID(),email);
								...
								<div class="product_buttons">
									<div class="text-right d-flex flex-row align-items-start justify-content-start">	
										...
										<div><div>
										 <a  href="product.jsp?value=<%=sweets.get(index_Prod).getProdID() %>" style="font-size:32px;border:none; background-repeat:no-repeat;background: transparent; ">  
								...
								<div class="product_buttons">
									<div class="text-right d-flex flex-row align-items-start justify-content-start">											
						                ...
						                <input type="hidden" value="<%=  sweets.get(index_Prod).getProdID()%>" name="prod_id">
										<input type="hidden" value=<%= jspName %> name="jspPage" >	
									    <a href="product.jsp?value=<%=sweets.get(index_Prod).getProdID()%>"  
										...
										<div class="product_button product_fav text-center d-flex flex-column align-items-center justify-content-center">
											<div><div>
											      <form action="<%=request.getContextPath()%>/LikeProductServlet" method="post">
											        <input type="hidden" value="<%= email %>" name="mailCus">
						                            <input type="hidden" value="<%= sweets.get(index_Prod).getProdID()%>" name="prodidLiked">
						                            <input type="hidden" value=<%= jspName %> name="jspPage" >											         
						...
						<div class="col-lg-4 footer_col">
							<div class="footer_about">
								<div class="footer_logo">
									<a href="#">
										<div class="d-flex flex-row align-items-center justify-content-start">
											 <form action="<%=request.getContextPath()%>LIServlet"><div><img src="view/images/logo5.png" alt="" height="80" width="130"></div></form>
											<div></div>
										</div>
									</a>
								</div>
