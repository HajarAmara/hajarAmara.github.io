...
<%@ page import=" java.io.UnsupportedEncodingException,com.hajar.zaki.dao.UserDAO, com.hajar.zaki.dao.UserDAOImpl, com.hajar.zaki.dao.OrderDAO,
 				  com.hajar.zaki.model.User , utils.*, java.sql.*, java.util.*,com.hajar.zaki.dao.ProductDAO, com.hajar.zaki.model.Product,
 				  com.hajar.zaki.db.DBConnect,com.hajar.zaki.model.Order , com.hajar.zaki.model.ProductPicture, com.hajar.zaki.dao.PicturesDao, java.io.*, java.util.Base64" %>
 <%@page import="com.hajar.zaki.model.Customer, com.hajar.zaki.dao.SupplierDAO,com.hajar.zaki.dao.ProductCustomerDAO,com.hajar.zaki.model.ProductCustomer,  com.hajar.zaki.dao.CustomerDAO, com.hajar.zaki.model.Supplier "%>
...
<div class="menu">
	...
	<div class="menu_nav">
		<ul>
		            <li class="active"><a href="category.jsp?userName=<%=userName%>&email=<%=email%>"  >מנה ראשונה </a></li>
	...
	<header class="header">
		...
		<div class="header_right d-flex flex-row align-items-center justify-content-start ml-auto">
			...
			<nav class="main_nav">
			 ...
			 <a href="chat.jsp?pageJsp=category" 
		...
		<div class="home">
			<div class="home_container d-flex flex-column align-items-center justify-content-end">
				<div class="home_content text-center">
					<div class="home_title">מנה ראשונה</div>
		...
		<div class="products">
			<div class="container">
				...
				<% if(email != null && sup){ %>
					...
					ArrayList<Product> appets_sup= prodDao.getAllAPPET_SUP(email);
					int valuesLength=appets_sup.size()-1;
				    InputStream sImage;
					int index_Prod=0;
				    for(index_Prod=0; index_Prod<=valuesLength;index_Prod++){
					  PicturesDao picDao= new PicturesDao();
					  ProductPicture prodPic = picDao.getPicturePerProductId(appets_sup.get(index_Prod).getProdID());
					  String imageBase64="";
						...
						<div class="product">
							...
							<div class="product_content">
								<div class="product_info d-flex flex-row align-items-start justify-content-start">
									<div>
										<div>
											<div class="product_name"><a href="product.jsp?value=<%=appets_sup.get(index_Prod).getProdID()%>">
											 <% 
										        out.println(appets_sup.get(index_Prod).getName() ); %> </a></div>
											<div class="product_category">המחיר / السعر  </div>
										</div>
									</div>
									<div class="ml-auto text-right">
										<!-- div class="rating_r rating_r_4 home_item_rating"><i></i><i></i><i></i><i></i><i></i></div-->
										<div class="product_price text-right">₪<%out.println(appets_sup.get(index_Prod).getPiece_price() ); %> </div>
									</div>
								</div>
				...
				<div class="row products_row ">
					 ...
					 <% 
					ProductDAO prodDao=new ProductDAO();
					ArrayList<Product> appetizers= prodDao.getAllAPPETIZERS();
					int valuesLength=appetizers.size()-1;
				    InputStream sImage;
					int index_Prod=0;
				    for(index_Prod=0; index_Prod<=valuesLength;index_Prod++){
					  PicturesDao picDao= new PicturesDao();
					  ProductPicture prodPic = picDao.getPicturePerProductId(appetizers.get(index_Prod).getProdID());
					  String imageBase64="";
					  if(prodPic != null && prodPic.getImg()!= null){
						  System.out.print("test into index img " + imageBase64);
						  imageBase64 = new String(Base64.getEncoder().encode((prodPic.getImg())));
							...
							<div class="product_content">
								<div class="product_info d-flex flex-row align-items-start justify-content-start">
									<div>
										<div>
											...
											<div class="product_name"><a href="product.jsp?value=<%=appetizers.get(index_Prod).getProdID()%>"> <% 
										     out.println(appetizers.get(index_Prod).getName() ); %> </a></div>
											<div class="product_category">המחיר / السعر  </div>
										</div>
									</div>
									<div class="ml-auto text-right">
										<!-- div class="rating_r rating_r_4 home_item_rating"><i></i><i></i><i></i><i></i><i></i></div-->
										<div class="product_price text-right">₪<%out.println(appetizers.get(index_Prod).getPiece_price() ); %> </div>
									</div>
								</div>
										<%  if( appetizers.get(index_Prod) != null){
								      ProductCustomerDAO pcdao = new ProductCustomerDAO(); 
									  ProductCustomer pc = new ProductCustomer(); 
									  pc = pcdao.getProductLike(appetizers.get(index_Prod).getProdID(),email);
								...
								<div class="product_buttons">
									<div class="text-right d-flex flex-row align-items-start justify-content-start">										
										...
										<div><div>
										 <a  href="product.jsp?value=<%=appetizers.get(index_Prod).getProdID() %>" style="font-size:32px;border:none; background-repeat:no-repeat;background: transparent; ">  
								...
								<div class="product_buttons">
									<div class="text-right d-flex flex-row align-items-start justify-content-start">	
						                ...
						                <input type="hidden" value="<%=  appetizers.get(index_Prod).getProdID()%>" name="prod_id">
										<input type="hidden" value=<%= jspName %> name="jspPage" >	
									    <a href="product.jsp?value=<%=appetizers.get(index_Prod).getProdID()%>"  
										...
										<div class="product_button product_fav text-center d-flex flex-column align-items-center justify-content-center">
											<div><div>
											      <form action="<%=request.getContextPath()%>/LikeProductServlet" method="post">
											        <input type="hidden" value="<%= email %>" name="mailCus">
						                            <input type="hidden" value="<%= appetizers.get(index_Prod).getProdID()%>" name="prodidLiked">
						                            <input type="hidden" value=<%= jspName %> name="jspPage" >											         
